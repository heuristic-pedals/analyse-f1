...
fig.update_layout(
    title=dict(  # noqa: C408
        text="<b>2025 Constructors Fastest Lap Championship<b>",
        font=dict(size=xlarge_font),  # noqa: C408
        y=0.945,
    )
)
...
# get starting location and adjust axes
grid_line_y = len(constructors_standings) * v_spacing
fig.update_xaxes(range=[-h_padding, 2 * grid_width + h_spacing + h_padding])
fig.update_yaxes(range=[-v_spacing, grid_line_y + v_spacing])


for i, constructor in constructors_standings.iterrows():
    # calculate the column
    col = i % 2
    ...
    fig.add_annotation(
        ...
        font=dict(size=med_font, color=f"#{constructor['TeamColor']}"),  # noqa: C408
        x=name_x,
        y=text_y,
        xanchor="left",
        yanchor="middle",
    ...
    pts_value = constructor["FastestLapPoints"]
    fig.add_annotation(
        text=(
            f"{pts_value:.0f}pts"
            if pts_value.is_integer()
